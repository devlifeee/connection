...
const RegistrationScreen = ({ onRegister }: Props) => {
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center p-4 overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-[#0a0a0a] text-white' : 'bg-[#f5f5f7] text-gray-900'}`}>
      ...
      <div className={`w-full max-w-[440px] relative z-10 backdrop-blur-2xl border rounded-[32px] p-8 sm:p-10 flex flex-col items-center gap-8 transition-all duration-500 ease-out ${
        isDarkMode 
        ...
        <div className="w-full space-y-5">
          <div className="space-y-1.5">
            <Input
              ...
              className={`h-[56px] border rounded-2xl text-[17px] px-6 font-medium transition-all duration-200 text-center tracking-wide focus:ring-[4px] ${
                isDarkMode 
          ...
          <div className="relative group">
            <div className={`relative flex items-center border rounded-2xl px-5 py-4 shadow-sm transition-all ${
              ...
              <div className="flex-1 flex flex-col min-w-0 mr-3 text-center sm:text-left">
                ...
                <span className={`font-mono text-[15px] tracking-tight truncate select-all font-medium ${isDarkMode ? 'text-white/90' : 'text-gray-800'}`}>
                  {nodeId}
                </span>
              </div>
              <button
        ...
        <div className="w-full space-y-6 mt-4">
          <Button
            ...
            disabled={!name.trim()}
            className={`w-full h-[60px] text-[18px] font-bold tracking-wide rounded-[26px] active:scale-[0.98] transition-all duration-300 hover:brightness-110 disabled:opacity-30 disabled:shadow-none disabled:cursor-not-allowed ${
              isDarkMode 
      ...
      {/* Version info footer */}
      <div className={`absolute bottom-8 text-[10px] font-mono tracking-widest mix-blend-plus-lighter ${isDarkMode ? 'text-white/10' : 'text-gray-400/50'}`}>
